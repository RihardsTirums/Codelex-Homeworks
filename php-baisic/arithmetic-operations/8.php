...
function baseSalary ($basePay,$hoursWorked) {
    $maxHours = 40; // Per week
    $minWage = 8;
    $overTime = 1.5;
    if($basePay < $minWage || $hoursWorked > 60){
        echo "Error";
    } elseif ($hoursWorked > $maxHours){
        $totalSalary = $basePay * $maxHours + $overTime * $basePay * ($hoursWorked - $maxHours);
        echo $totalSalary;
    } elseif ($hoursWorked == $maxHours) {
